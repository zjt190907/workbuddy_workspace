...
// Forward declaration from sprites_render.cpp
extern void drawExpressionSprite(Adafruit_ST7789& tft, AnimState& st);
extern SpriteSet exprToSprite(Expression ex);
...
// This is slower than hardware SPI but avoids pin-mapping issues on ESP32-C3
...
inline uint16_t rgb888_to_rgb565(uint32_t rgb888) {
    ...
    // R: 8-bit → 5-bit (top 5 bits), G: 8-bit → 6-bit, B: 8-bit → 5-bit
...
void handleExpr() {
    ...
    if (name.length() == 0 && server.hasArg("name") == false) {
        // Try path-based: /api/expr?name=happy or /api/expr/happy
        // Check URL path for /api/expr/<name>
...
void handleSpeed() {
    ...
    if (val.length() > 0) {
        ...
        // Try path-based: /api/speed/<value>
        String uri = server.uri();
...
void handleBg() {
    ...
    if (val.length() >= 6) {
        // Convert web RGB888 hex to ST7789 RGB565
...
void handleDisplay() {
    ...
    if (val.length() == 0) {
        // Try path-based: /api/display/<0|1>
        String uri = server.uri();
...
void handlePixel() {
    ...
    // Fallback: try path-based /api/pixel/<x>/<y>/<color>
...
void initWebServer() {
    ...
    // These are handled by the same handlers via URI parsing
    server.onNotFound([]() {
        String uri = server.uri();
...
void loop() {
    ...
    // Use sprite renderer if sprite exists for this expression
    uint32_t now = millis();
    SpriteSet ss = exprToSprite(anim.current);
    if (ss != SPR_INVALID) {
        drawExpressionSprite(tft, anim);
    } else {
