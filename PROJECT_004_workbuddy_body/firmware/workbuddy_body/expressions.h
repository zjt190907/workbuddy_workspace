...
void drawSmile(Adafruit_ST7789 &tft, int16_t cx, int16_t base_y,
...
void drawFrown(Adafruit_ST7789 &tft, int16_t cx, int16_t base_y,
    int16_t w, int16_t h, uint16_t c) {
...
void drawIdle(Adafruit_ST7789 &tft, AnimState &st, bool blink) {
...
void drawHappy(Adafruit_ST7789 &tft, AnimState &st) {
    clearScreen(tft, st.bg_color);
...
void drawHaha(Adafruit_ST7789 &tft, AnimState &st) {
    clearScreen(tft, st.bg_color);
...
void drawThinking(Adafruit_ST7789 &tft, AnimState &st) {
    clearScreen(tft, st.bg_color);
...
void drawWorking(Adafruit_ST7789 &tft, AnimState &st) {
    clearScreen(tft, st.bg_color);
...
void drawDone(Adafruit_ST7789 &tft, AnimState &st) {
    clearScreen(tft, st.bg_color);
...
void drawSleeping(Adafruit_ST7789 &tft, AnimState &st) {
    clearScreen(tft, st.bg_color);
...
// --- ERROR: X eyes, frown ---
void drawError(Adafruit_ST7789 &tft, AnimState &st) {
    clearScreen(tft, st.bg_color);
...
void drawBoring(Adafruit_ST7789 &tft, AnimState &st) {
    clearScreen(tft, st.bg_color);
...
// --- LOGO: Boot screen ---
void drawLogo(Adafruit_ST7789 &tft, AnimState &st) {
    clearScreen(tft, st.bg_color);
    tft.setTextSize(5);
    tft.setTextColor(st.eye_color, st.bg_color);
...
void drawCanvas(Adafruit_ST7789 &tft, AnimState &st) {
    // Managed by web pixel commands
}

// ==================== DISPATCHER ====================

void drawExpression(Adafruit_ST7789 &tft, AnimState &st, bool blink) {
    switch (st.current) {
...
const char* getExpressionName(Expression e) {
    switch (e) {
...
Expression getExpressionByName(const String &name) {
